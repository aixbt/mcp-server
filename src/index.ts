...
const log = {
  ...
  request: (method: string, url: string, params?: any) => {
    console.error(`[REQUEST] ${method} ${url}`, params ? `params: ${JSON.stringify(params)}` : '');
  },
  response: (status: number, url: string, data: any) => {
    console.error(`[RESPONSE] ${status} ${url} data: ${JSON.stringify(data).substring(0, 200)}...`);
  }
};
...
server.tool(
  ...
  {
    name: z.string().min(2).max(50).describe("Project or token name (i.e. ETH)"),
    limit: z.number().min(1).max(50).describe("Maximum number of summaries"),
  },
  async ({ name, limit }) => {
    log.info(`Executing tool: list-project-latest-summaries with name=${name}, limit=${limit}`);
    try {
      ...
      log.debug(`Found project: ${project.name}, summaries count: ${project.summaries?.length || 0}`);
      
      const summaries = project.summaries || [];
      const limitedSummaries = summaries.slice(0, limit);
      log.debug(`Returning ${limitedSummaries.length} summaries for ${project.name}`);
